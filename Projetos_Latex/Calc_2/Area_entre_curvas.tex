...
\begin{example}
  ...
  \begin{figure}[H]
  \centering
    \includegraphics[width=0.7\linewidth]{area_entre_y=x+4_e_y=raiz_de_0.5x.png}
...
Conforme visto na seção\, \ref{sec:integral-riemann} sobre integral de Riemann, quando uma função $f(x)$ é contínua e positiva em um intervalo $[a,b]$, a área é calculada entre a curva e o eixo $x$. Neste caso, o eixo $x$ atua como o limite inferior ($y = 0$).
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \node[anchor=west] at (axis cs:1.35,0.7) {$\sin(x)$};
    \end{axis}
  \end{tikzpicture}
...
\]
A área limitada superiormente por \(y=f(x)\), inferiormente por \(y=g(x)\) e lateralmente por \(x=a\) e \(x=b\), sem que as curvas se cruzem no intervalo.

Não importa se \(f\) e \(g\) estão acima ou abaixo do eixo \(x\). O que importa é somente qual delas está \emph{acima da outra}. Mesmo que \(f(x)\) e \(g(x)\) sejam negativos, a área continua sendo \(\int ( \text{superior} - \text{inferior})\,dx\).
\end{example}
...
\subsection{Curvas que se entrecruzam}
