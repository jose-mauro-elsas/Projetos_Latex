...
\begin{column}{0.48\textwidth}
\begin{itemize}
    \item causas diversas reduziram a vazão do rio e consequentemente o aporte sedimentar e aumentaram o avanço da cunha salina;
...
\begin{columns}[T]
    \begin{column}{0.48\textwidth}
        \begin{itemize}
            ...
            \item Cada unidade é georreferenciada com o uso de Estação Total e DGPS e RTK (Real-time Kinematics), o que garante que a obra seguirá muito de perto o projeto geomorfológico.
...
\begin{columns}[T]
    \begin{column}{0.48\textwidth}
        \begin{itemize}
            \item Independentemente do produto (pedras ou Core-Loc\texttrademark) que se use para construção do quebra-mar, algumas configurações foram utilizadas com sucesso anteriormente, 

            \item em (a) temos espigões numa configuração típica. 

            \item em (b), temos um arranjo típico de quebra-mares ao longo da praia,

            \item em (c) vemos a seção transvesal de um quebra-mar.

            \item e em (d), resposta de linha de costa prevista para espigões em T em \emph{Upham Beach, Florida}
        \end{itemize}
    \end{column}

    \begin{column}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth,height=0.72\textheight,keepaspectratio]{configs.png}
