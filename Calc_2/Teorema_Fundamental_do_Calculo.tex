...
\subsection*{Aplicações do Teorema Fundamental do Cálculo}

O Teorema Fundamental do Cálculo (TFC) possui duas faces complementares:
\begin{enumerate}
    \item Se \(F(x)=\displaystyle \int_a^x f(t)\,dt\) e \(f\) é contínua, então
    \[
    F'(x)=f(x).
    \]
    \item Se \(f\) é contínua em \([a,b]\), então
    \[
...
Muitos exercícios aparentemente distintos são, na verdade, aplicações diretas ou consequências estruturais do TFC.

\subsubsection*{1. Limites envolvendo integrais}

Considere o limite
\[
\lim_{x\to 0^+}
\frac{\int_{0}^{\sin x} e^{t^2}\,dt}{\sin x}.
\]
...
Como \(F(0)=0\), o limite acima pode ser interpretado como
\[
\lim_{u\to 0^+} \frac{F(u)-F(0)}{u-0},
\]
isto é, a definição da derivada de \(F\) em \(0\). Logo,
\[
\lim_{x\to 0^+}
\frac{\int_{0}^{\sin x} e^{t^2}\,dt}{\sin x}
=
...
\textbf{Interpretação:} trata-se de uma aplicação direta da primeira parte do TFC.

\subsubsection*{2. Mudança de variável como consequência do TFC}
...
Fazendo a substituição \(u=\sin^2 x\), temos \(du=\sin(2x)\,dx\), e a integral torna-se
\[
\int_0^{1/4} e^u\,du.
\]

A justificativa teórica dessa substituição repousa no fato de que
\[
\frac{d}{dx} \big( F(f(x)) \big)
=
F'(f(x))\, f'(x),
\]
ou seja, na regra da cadeia aplicada à primitiva — resultado cuja validade depende do TFC.

\textbf{Interpretação:} a mudança de variável em integrais definidas é uma consequência estrutural do Teorema Fundamental do Cálculo combinada com a regra da cadeia.
...
f'(x)\,F(f(x)),
\]
é recomendável investigar se o problema pode ser resolvido por meio do Teorema Fundamental do Cálculo.
...
\begin{exercicio} Esse exercício foi questão de prova (primeira). Na integral abaixo será feita uma mudança de variaveis, portanto os limites de integração serão também modificados.
  \begin{align*}
    &\int_{0}^{\frac{\pi}{6}}\sin(2x)e^{\sin^2(x)}dx\\
    &u=\sin^2(x)\\
    &du=2\sin(x)\frac{d}{dx}\sin(x)\\
    &du=2\sin(x)\cos(x)\;dx\\
    &\text{mas,}\\
    &2\sin(x)\cos(x)=\sin(2x)\\
    \therefore&\quad du=\sin(2x)\\
  \end{align*} 
os novos limites de integração:
\begin{align*}
  \text{A integral é:}&\int e^u\,du\\
  \text{quando}\,x&=0, u=sin^2(0)=0\\
  \text{e quando}\,x&=\frac{\pi}{6},\, u=\sen\Big(\frac{\pi}{6}\Big)=\frac{1}{2}\\
\end{align*}
Então com os limites temos:
\begin{align*}
  &\int_0^{\frac{1}{2}} e^u\,du\\
  &e^u\Big|_0^\frac{1}{2}=\sqrt{e}-0=\sqrt{e}\\
\end{align*}
\end{exercicio}

\begin{exercicio} Esse exercício também foi questão de prova.
  Aqui o segredo é perceber que função definida por integral avaliada em $\sin(x)$

 \begin{align*}
   &\lim_{\to{0+}}\frac{\int_{0}^{\sin(x)}e^{t^2}\,dt}{\sin(x)}\\
 \end{align*}
 Vamos tentar resolver a integral.
 \begin{align*}
  &\int_{0}^{\sin(x)}e^{t^2}\,dt\\
  &F(u)=
 \end{align*}
...
\subsection{Demonstração Formal}
...
\subsection{Caso com Limite Variável Geral}
...
\subsection{Regra Geral (Regra de Leibniz)}
...
\subsection{Exemplo por somas superior/inferior}
...
\subsection{Relação do TFC com as integrais definidas}
Se \(F\) é uma primitiva de \(f\) (\(F'=f\)), então:
\[
\int_a^b f(x)\,dx = F(b)-F(a).
\]

\subsection{Integrais impróprias (observação)}
...
\subsection{O Teorema Fundamental do Cálculo (Parte 1)}
...
\subsection{Caso Geral -- Ambos os limites são funções}
